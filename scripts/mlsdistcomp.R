...
init_global_context <- function() {
  if (.Platform$OS.type == "windows") { 
    mlsdbsecrets_csv <<- paste0(datafolder_windows,'/',db_info_filename)
    computeContext <<- list(
      datafolder = datafolder_windows
    )
    tokenCacheFilePath <<- paste0(computeContext$datafolder, "/mlstokencache.csv")
  } else if (Sys.info()["sysname"] == "Darwin") {
    "mac" 
  } else if (.Platform$OS.type == "unix") { 
    mlsdbsecrets_csv <<- paste0(datafolder_linux,'/',db_info_filename)
    computeContext <<- list(
      datafolder = datafolder_linux
    )
    tokenCacheFilePath <<- paste0(computeContext$datafolder, "/mlstokencache.csv")
  } else {
    stop("Unknown OS")
...
MRSDistCompBase <- R6Class(
  ...
  public = list (
    initialize = function() {
      ...
      db_info_df <- read.csv(file=mlsdbsecrets_csv, header=TRUE, sep=",")
      if(nrow(df) != 1) stop('Database info csv has no valid content')
      private$connStr <-
        sprintf(
      ...
      ifelse(!dir.exists(file.path(computeContext$datafolder)), dir.create(computeContext$datafolder), FALSE)
      invisible(self)
    },
    finalize = function() {
    ...
    getAADToken = function(sitename,
      ...
      if(!file.exists(tokenCacheFilePath)){
        ...
        write.table(tokendf, file=tokenCacheFilePath, append = FALSE, row.names = FALSE,
                    col.names = TRUE, quote = TRUE, sep = ",")
      }

      # Check if token exists for site
      tokencachedf = as.data.frame(read.table(file=tokenCacheFilePath, header=TRUE, sep=",", stringsAsFactors = FALSE))
      ...
      if(is.null(sitedf) || nrow(sitedf) == 0) {
        ...
        write.table(as.data.frame(currentSite, stringsAsFactors = FALSE), file = tokenCacheFilePath, append = TRUE, row.names = FALSE,
        ...
        write.table(tokencachedf, file = tokenCacheFilePath, append = FALSE, row.names = FALSE,
                    col.names = TRUE, quote = TRUE, sep = ",")
      }
    ...
    getAADTokenFromCache = function(sitename){
      ...
      if(!file.exists(tokenCacheFilePath)){
        stop(sprintf("Token cache file '%s' not found", tokenCacheFilePath))
      }

      tokencachedf = read.table(file=tokenCacheFilePath, header=TRUE, sep=",")
...
DataSources <- R6Class(
  ...
  public = list(
    ...
    getDataSourcesByName = function(nameFilter) {
      query <- NULL
      if (!is.null(nameFilter)) {
        query <- sprintf("Name like '%s'", nameFilter)
      }
...
ComputationInfoJob <- R6Class(
  ...
  public = list(
    ...
    processJob = function(jobId,
      ...
      workerObj = NULL
      filePath = paste0(computeContext$datafolder, "/", jobId, ".rds")

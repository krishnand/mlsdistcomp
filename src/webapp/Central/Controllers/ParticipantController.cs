        ...
        public async Task<ActionResult> Index(string name, string clientid, string clientsecret, string tenantid, string url)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    HttpContent content = new StringContent(JsonConvert.SerializeObject(new { name = name, clientid = clientid, clientsecret = clientsecret, tenantid = tenantid, url = url}), 
                    ...
                    //
                    HttpClient client = new HttpClient();
                    HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, AzureAdOptions.Settings.CentralRegistryBaseAddress + "/api/RegisterParticipant");
                    request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", result.AccessToken);
